...
export const SAMPLE_INDUSTRY: IndustryResult = {
  summary: "여러 팀이 함께 쓰는 업무 도구를 구독형으로 파는 시장입니다. AI 기능은 따로 파는 제품이 아니라 이미 쓰는 화면 안으로 들어가고 있습니다.",
  social: "반복 업무에 시간을 쓰는 실무자의 부담을 줄이고, 작은 조직도 큰 조직과 비슷한 도구를 쓰게 합니다. 반대로 도구가 늘수록 도입과 교육 비용이 커지고, 업무 기록이 회사 밖 서버에 쌓이는 문제도 함께 생깁니다.",
  personalQuestions: [
    "반복 업무 때문에 시간을 잃어 본 적이 있나요? 어떤 일이었나요?",
    "도구를 만드는 일과 도구를 쓰는 일 중 어느 쪽에 더 관심이 있나요?",
    "비슷한 조건의 다른 산업과 비교할 때 이 산업에서 중요하게 보는 차이는 무엇인가요?",
  ],
...
export const SAMPLE_COMPANY: CompanyResult = {
  summary: "팀의 반복 업무를 데이터로 찾아 줄여 주는 가상의 B2B SaaS 기업입니다. AI 기능을 기존 업무 화면 안에 넣는 방향을 택했다고 가정합니다.",
  social: "여러 도구에 흩어진 업무 기록을 모아 실무자가 같은 일을 두 번 하지 않게 합니다. 한계로는 기록을 모으는 과정에서 직원 활동이 관리자에게 더 많이 보이게 되는 점이 있습니다.",
  personalQuestions: [
    "이 회사가 줄이려는 반복 업무를 직접 겪거나 가까이서 본 적이 있나요?",
    "그 문제를 푸는 일의 어떤 부분에 관심이 있나요?",
    "이 회사를 다른 회사와 비교할 때 본인이 중요하게 보는 차이는 무엇인가요?",
  ],
